...
- **silenceWorker** — runs every 10 seconds, finds sensors with `last_seen_at` older than 2 minutes, creates `pattern_absence` anomaly and marks sensor `silent`
- **escalationWorker** — runs every 10 seconds, finds critical open alerts older than 5 minutes that have not been escalated, reassigns to supervisor, writes to `escalation_log`
